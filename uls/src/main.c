...
int main(int argc, char *argv[]) {
    ...
    //mx_printint(uid);
    //key = getpwuid(uid);
    //mx_printstr(key->pw_passwd);
    //mx_printstr("\n");
    mx_printstr(key->pw_name);
    mx_printchar('\n');
    mx_printstr(key->pw_dir);
    mx_printchar('\n');
    //struct group * new_group;
    new_group = getgrgid(gid);
    mx_printstr(new_group->gr_name);
    mx_printchar('\n');
    mx_printstr(new_group->gr_passwd);
    mx_printchar('\n');
    struct stat buff;
    stat("src", &buff);
    //mx_printint(buff.st_ino);
    //mx_printint(buff.st_uid);
    //mx_printint(buff.st_gid);
    //mx_printint(buff.st_blksize);
    //lstat("uls", &buff);*/
    //key = getpwuid(1000);
    //mx_printstr(key->pw_name);
    //mx_printchar('\n');
